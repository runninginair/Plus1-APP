import React from "react";
import "./index.css";
import HomeNav from "../home-nav/navhome";

const HomeComponent = () => {
    return (
        <div>
            <center><br />
                {<HomeNav/>}<br />
                <img className="rounded-image" height={100} width={100}
                    src="data:image/png;base64,"
                    alt="+1 logo" />
                <h2>
                    <span className="logo fw-bold"> Plus1 </span>
                    Workout Track & Eval Application</h2>
                <img height={180} src="https://www.swiss-miss.com/wp-content/uploads/legacy/photos/uncategorized/2008/08/06/image-480x133.jpg" alt=""/>
                <br /><br /><br /><br /><br /><br /><br />
                <p>Developers: Tao Jin, Meng Wang, Mingyang Wang, Jin Zhang.</p>
                <footer>
                    &copy; Northeastern University cs5500 Spring 2023. All rights reserved.
                </footer><br />
            </center>
        </div>
    );
}

export default HomeComponent;